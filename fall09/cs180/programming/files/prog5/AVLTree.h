...
protected:
  ...
  struct AVLNode {
	...
	AVLNode(const Object& e = Object(), int h = 1, Node* p = 0, Node* l = 0, Node* r = 0) : element(e), parent(p), left(l), right(r), height(h) { }	
...
private:
	...
	void clearRecurse(const AVLNode* v) {
		if (v->left != 0) 
			clearRecurse(v->left);
		if (v->right != 0)
	...
	Node* cloneRecurse(const Node* v) {
		Node* n = new AVLNode(v->element, v->height);
		if (v->left != 0) {
...
public:
	...
	//you may assume only unique items will be inserted into the tree to simplify your code
